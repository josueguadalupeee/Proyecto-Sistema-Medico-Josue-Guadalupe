...
class ExamenSolicitadoAdmin(admin.ModelAdmin):
    list_display = ('nombre_examenes', 'paciente', 'fecha_solicitud', 'estado')
    search_fields = ('nombre_examen', 'paciente__nombre')
    def nombre_examenes(self, obj):
        # Mostrar los exámenes seleccionados (campo ManyToMany)
        return ", ".join([str(examen) for examen in obj.nombre_examen.all()])
    nombre_examenes.short_description = 'Nombre del Examen'
